...
citibike_tidy %>% 
  ...
  group_by(start_month, start_day) %>% 
    mutate(mean_triplength = mean(trip_minutes)) %>% 
    ggplot(aes(x = start_day, y = mean_triplength, color = start_month)) +
...
>>>>>>> ad4ff70bb9b281fea110f520066ea93dc5d4023d
```

#### Average ride length

```{r age_length_plot, include=FALSE}
...
citibike_tidy %>% 
  ...
  group_by(start_day) %>% 
  mutate(mean_triplength = mean(trip_minutes),
         max_trip = max(trip_minutes),
         min_trip = min(trip_minutes)) 

citibike_length %>% 
  plot_ly(x = ~start_day, y = ~max_trip, type = 'scatter', mode = 'lines',
        line = list(color = 'transparent'),
        showlegend = FALSE, name = 'Longest') %>%
  add_trace(y = ~min_trip, type = 'scatter', mode = 'lines',
            fill = 'tonexty', fillcolor='rgba(0,100,80,0.2)', line = list(color = 'transparent'),
            showlegend = FALSE, name = 'Shortest') %>%
  add_trace(x = ~start_day, y = ~mean_triplength, type = 'scatter', mode = 'lines',
            line = list(color='rgb(0,100,80)'),
            name = 'Average') %>%
  layout(title = "Average trip (with highest and shortest), by day of the month",
         paper_bgcolor='rgb(255,255,255)', plot_bgcolor='rgb(229,229,229)',
         xaxis = list(title = "Day of the month",
                      ...
                      ticks = 'outside',
                      zeroline = FALSE),
         yaxis = list(title = "Length of trip (minutes)",
                      gridcolor = 'rgb(255,255,255)',
                      showgrid = TRUE,
                      showline = FALSE,
                      showticklabels = TRUE,
